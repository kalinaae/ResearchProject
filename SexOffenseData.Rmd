...
# Create a line graph
p <- ggplot(df_crime_monthly, aes(x = factor(month_name, levels = month.name[c(7:12, 1:6)]), y = count, group = Year, color = as.factor(Year))) +
  geom_line() +
  geom_point() +
...
#df_crime_monthly <- df_crime_monthly %>% 
  as.Date(Month, format="%m")

#str(df_crime_monthly)
#$Month <- as.Date(df_crime_monthly$Month, format="%m")
````
