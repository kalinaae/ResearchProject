...
viz_data$Inc_Month <- month(viz_data$startDate)
viz_data$Inc_Year <- year(viz_data$startDate)
...
# Create a line graph
p <- ggplot(df_crime_monthly, aes(x = factor(month_name, levels = month.name[c(7:12, 1:6)]), y = count, group = Inc_Year, color = as.factor(Inc_Year))) +
  geom_line() +
  geom_point() +
...
month_year <- ggplot(df_crime_monthly, aes(x = month_name, y = count, fill= as.factor(Inc_Year))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  labs(title = "Sexual Offense Count by Month", x = "Months", y = "Count") +
  theme_minimal()

month <- ggplot(df_crime_monthly, aes(x = month_name, y = count, fill= month_name)) +
